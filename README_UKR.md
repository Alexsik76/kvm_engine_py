...
Свіжий образ Raspberry Pi OS вішає на UART, яким користується передня панель на
RP2040, серійну консоль входу (`serial-getty@ttyAMA0`). Якщо її лишити, ця
консоль безперервно шле в мікроконтролер запрошення логіну й конкурує з двигуном
за порт — панель відповідає `unknown_command` і не показує PWR/HDD. Крок
`10_boot_config` тепер зупиняє й маскує цей сервіс (саме маскує — звичайний
disable скасовується systemd-генератором при наступному завантаженні). Зміна діє
